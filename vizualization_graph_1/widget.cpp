...
void Widget::printEllipseGraph(){
    int lengthEllipseNodes = 150; // расстояние между вершинами на одном уровне
    ...
    QVector <class Node> printedNodes;
    ...
    bool isEnd = false;
    while(!isEnd){
        QVector <class Node> readyToPrint;
        for(int i = 0; i < count_nodes; ++i){
        ...
        if(!readyToPrint.size())
            isEnd = true;
        QBitArray placeNodes(readyToPrint.size());
        if(currentR * 2 < 400)
            currentR *= 2;
        else
            currentR += 400; // увеличение радиуса на 400, если новый радиус не выше этой величины
        for(int i = 0; i < readyToPrint.size(); ++i)
            placeNodes[i] = false;
        for(size_t i = 0; i < readyToPrint.size(); ++i){
            int minLengthPath = 2147483647; // максимальное число в int для поиска минимума
            double minAngle = 0;
            int minJ = 0;
            ...
            for(int j = 0; j < readyToPrint.size(); ++j){
                ...
                for(size_t k = 0; k < printedNodes.size(); ++k){
                    if(connection_matrix[printedNodes[k].getNode_num()][readyToPrint[i].getNode_num()])
                        currentMinLengthPath += sqrt(pow((readyToPrint[i].getX() - printedNodes[k].getX()), 2) +
                                                     pow((readyToPrint[i].getY() - printedNodes[k].getY()), 2));
                }
